...
_PEP610_USER_PASS_ENV_VARS_REGEX = re.compile(r"^\$\{[A-Za-z0-9-_]+\}(:\$\{[A-Za-z0-9-_]+\})?$")
...
class ArchiveInfo:
    ...
    def _from_dict(cls, d: Mapping[str, Any]) -> Self:
        ...
        if hashes is not None and not all(isinstance(h, str) for h in hashes.values()):
            raise DirectUrlValidationError("Hash values must be strings", context="hashes")
...
class DirectUrl:
    ...
    def _from_dict(cls, d: Mapping[str, Any]) -> Self:
        ...
        if (
            bool(direct_url.vcs_info) + bool(direct_url.archive_info) + bool(direct_url.dir_info)
        ) != 1:
            raise DirectUrlValidationError(
