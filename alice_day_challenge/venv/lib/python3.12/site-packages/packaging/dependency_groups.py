...
class CyclicDependencyGroup(ValueError):
    ...
    def __init__(self, requested_group: str, group: str, include_group: str) -> None:
        ...
        super().__init__(
            "Cyclic dependency group include while resolving " f"{requested_group}: {reason}"
        )
...
class DependencyGroupResolver:
        ...
        for item in parsed:
            if isinstance(item, Requirement):
                ...
                if include_group in self._include_graph_ancestors.get(group, ()):
                    errors.error(CyclicDependencyGroup(requested_group, group, item.include_group))
                else:
                    self._include_graph_ancestors[include_group] = (
                        *self._include_graph_ancestors.get(group, ()),
                        group,
                    )
                    resolved_group.extend(self._resolve(include_group, requested_group, errors))
    ...
    def _parse_group(
        ...
        if isinstance(raw_group, str):
            errors.error(
                TypeError(f"Dependency group {group!r} contained a string rather than a list.")
            )
            return ()

        if not isinstance(raw_group, Sequence):
            errors.error(TypeError(f"Dependency group {group!r} is not a sequence type."))
            return ()
        ...
        for item in raw_group:
            if isinstance(item, str):
                ...
                if tuple(item.keys()) != ("include-group",):
                    errors.error(
                        InvalidDependencyGroupObject(f"Invalid dependency group item: {item!r}")
                    )
                else:
...
def _normalize_group_names(
    ...
    for normed_name, names in original_names.items():
        if len(names) > 1:
            errors.error(
                DuplicateGroupNames(
                    "Duplicate dependency group names: " f"{normed_name} ({', '.join(names)})"
                )
            )
